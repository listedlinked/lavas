...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::LAVAS)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("LAVAS")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeLAVASAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeLAVASAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeLAVASAmount = strAnonymizeLAVASAmount.remove(strAnonymizeLAVASAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLAVASAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeLAVASAmount * COIN) nMaxToAnonymize = nAnonymizeLAVASAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeLAVASAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeLAVASAmount));
        strAnonymizeLAVASAmount = strAnonymizeLAVASAmount.remove(strAnonymizeLAVASAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeLAVASAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
