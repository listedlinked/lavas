...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LAVASMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("LAVASMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("LAVASMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lavas-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("LAVASMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running LAVASMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadLAVASMiner() exception");
    } catch (...) {
        LogPrintf("ThreadLAVASMiner() exception");
    }

    LogPrintf("ThreadLAVASMiner exiting\n");
}
