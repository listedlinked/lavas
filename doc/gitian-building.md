...
*Setup instructions for a gitian build of LAVAS using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LAVAS
...
Clone the git repositories for lavas and gitian and then checkout the lavas version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the lavas repository
...
To build LAVAS (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the lavas repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lavas/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/listedlinked/lavas
...
./bin/gbuild --commit lavas=${COMMIT} --url lavas=${URL} ../lavas/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lavas=${COMMIT} --url lavas=${URL} ../lavas/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lavas=${COMMIT} --url lavas=${URL} ../lavas/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lavas-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lavas-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/lavas-build.assert
```
...
[lavas/gitian.sigs](https://github.com/lavas-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
